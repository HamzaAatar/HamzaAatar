...
const filteredPosts = computed(() => {
  ...
  // In a real application, you would have tags in your blog posts frontmatter
  // For now, we'll simulate this using the post category and title keywords
  return posts.value.filter(post => {
    // Convert tag to lowercase for case-insensitive comparison
    const tagLower = tag.value.toLowerCase()
    
    // Check if tag appears in title, description, or category
    return (
      (post.category && post.category.toLowerCase().includes(tagLower)) ||
      (post.title && post.title.toLowerCase().includes(tagLower)) ||
      (post.description && post.description.toLowerCase().includes(tagLower))
    )
  })
})
...
useSeoMeta({
  ...
  ogDescription: () => `Browse our collection of articles related to ${tag.value}`,
})
</script> 